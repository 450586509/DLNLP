...
if __name__=='__main__':
    nb_class = 2
    model_file = './models/chinese_couprse_best_model.h5'
    total_test_acc =[]
    ...
    for k,(train_index,test_index) in enumerate(kfold):
        ...
        bilstm = Modes.CNN(embedding_para=embdedding,
                                   cnn_para = cnn_para,
                                   dense_para=dense_para)
        max_val_acc = 0 
        #early stopping
        earlystop = Modes.EarlyStop(max_count = 14)
        for epoch in range(200):
            history_data = bilstm.fit(X_train,
                                     y_train,
                                     batch_size=50,
                                     nb_epoch=1,
        ...
        del bilstm  # deletes the existing model
        bilstm = load_model(model_file)
        score, acc = bilstm.evaluate(X_test, y_test, batch_size=50, verbose=1, sample_weight=None)
        ...
        #get predict_y
        predict_y = bilstm.predict(X_test)
        #export result
        ...
        ResultAnalysis.comp_predict_lable(X = X_test,y=y_test,y_predict = predict_y,
                                          ...
                                          file_to_save='./result_analysis/chinese_zi_epoch_{}_cnn.txt'.format(k))
