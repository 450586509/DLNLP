...
if __name__=='__main__':
    nb_class = 2
    model_file = './chinese_ci_best_bl_cnn_model.h5'
    total_test_acc =[]
    tdg = TextDataGenerator(max_len = 120,
                            language='chinese',
                            is_char=False,
                            classes = nb_class
                            )
    embedding_file =  '/home/bruce/data/polyglot-zh.pkl'
    ...
    embdedding={'weights':embedding_weights,'drop_out':0.5,'input_dim':len(embedding_weights),'output_dim':64}
    lstm_para  ={'hidden':64,'return_sequences':True,'dropout_u':0,'dropout_w':0}
    ...
    for k,(train_index,test_index) in enumerate(kfold):
        ...
        max_val_acc = 0
        earlystop = Modes.EarlyStop(max_count=123)
        total_train_loss = []
        total_test_loss =[]
        for epoch in range(200):
            ...
            total_test_loss.append(val_loss)
            total_train_loss.append(train_loss)
            logger.info('total_test_loss={}'.format(total_test_loss))
            logger.info('total_train_loss={}'.format(total_train_loss))
            logger.info('runtime = {},epoch ={},train_acc={},train_loss={},val_acc={},val_loss={}'.format(k,epoch, train_acc, train_loss,val_acc, val_loss))
            if val_acc > max_val_acc:
                logger.info('saving model')
                max_val_acc = val_acc
                bl_cnn.save(model_file)
            logger.info('_count = {},_val_acc={}'.format(earlystop._count,earlystop._max_accuray))
        ...
        logger.info('total test acc ={}'.format(total_test_acc))
        
        #get predict_y
        predict_y = bl_cnn.predict(X_test)
        ...
        ResultAnalysis.comp_predict_lable(X = X_test,y=y_test,y_predict = predict_y,
                                          ...
                                          file_to_save='./result_analysis/chinese_ci_epoch_{}_blcnn.txt'.format(k))
